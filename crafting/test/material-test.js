...
describe("Material", function () {
  it("should have a name, price per unit, amount, and units", function () {
    var thread = new Material("thread", 0.25, 8, "spools");
    var fabric = new Material("fabric", 4, 3, "yards");

    assert.equal(thread.name, "thread");
    assert.equal(thread.price, 0.25);
  ...
  it("should not be able to use more material than available", function () {
    ...
    assert.equal(
      paint.useMaterial(12),
      "You don't have enough acrylic paint! Try using 7 bottles or less."
    );
    assert.equal(
      canvas.useMaterial(4),
      "You don't have enough canvas! Try using 2 sheets or less."
    );
  });
