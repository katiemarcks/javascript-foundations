...
describe("Golf Course", function () {
  ...
  it("should have some unique features", function () {
    var golfCourse1 = new GolfCourse("Arrowhead", "hard", 42, [
      "great views",
      "wildlife",
    ]);
    var golfCourse2 = new GolfCourse("Bay Hill", "hard", 11, [
      "water",
      "Arnies Place",
    ]);
  ...
  it("should accept golfers if there is room available", function () {
    var golfCourse = new GolfCourse("Bear Dance", "hard", 8, [
      "bear shaped green, views",
    ]);
  ...
  it("should not accept golfers if there is no room available", function () {
    var golfCourse = new GolfCourse("Bear Dance", "hard", 4, [
      "bear shaped green, views",
    ]);
    var golfer1 = new Golfer({ name: "Pat", handicap: 11 });
    var golfer2 = new Golfer({ name: "Ted", handicap: 13 });
    ...
    assert.equal(
      deniedCheckIn,
      "Sorry, we are currently booked! Please come back later."
    );
  });
});
...
describe("Golfer", function () {
  ...
  it("should be able to calculate their average score by adding the given par to their handicap", function () {
    ...
    assert.equal(
      golfer1.calculateAvg(par1),
      "I usually shoot a 76 on average."
    );
    assert.equal(
      golfer2.calculateAvg(par2),
      "I usually shoot a 85 on average."
    );
  });
  ...
  it("should become frustrated based on the course difficulty (HARD or MODERATE - golf is never easy)", function () {
    ...
    var golfCourse1 = new GolfCourse("Bear Dance", "hard", 44, [
      "elevation",
      "views",
    ]);
    var golfCourse2 = new GolfCourse("Willis Case", "moderate", 31, [
      "proximity to Denver",
      "layout",
    ]);
  ...
  it("should be able to marvel at the features of a golf course", function () {
    ...
    var golfCourse2 = new GolfCourse("Arrowhead", "hard", 42, [
      "undulating greens",
      "creative layout",
    ]);

    assert.equal(
      golfer4.marvel(golfCourse1),
      "I love the great views and wildlife on this course!"
    );
    assert.equal(
      golfer5.marvel(golfCourse2),
      "I love the undulating greens and creative layout on this course!"
    );
  });
