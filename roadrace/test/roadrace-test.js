...
describe("Roadrace", function () {
  ...
  it("should be able to set the race distance in miles", function () {
    ...
    assert.equal(
      race1Distance,
      `The Rock N Roll Half Marathon in Las Vegas is a 13.1 mile race.`
    );

    assert.equal(race2.distance, 26.2);
    assert.equal(
      race2Distance,
      `The City Park Marathon in Denver is a 26.2 mile race.`
    );
  });
  ...
  it("should add race to each participant's completed races when race is completed", function () {
    ...
    assert.equal(
      mike.completedRaces.includes("Rock N Roll Half Marathon"),
      true
    );
    assert.equal(
      pam.completedRaces.includes("Rock N Roll Half Marathon"),
      true
    );
    assert.equal(
      lazyHeather.completedRaces.includes("Rock N Roll Half Marathon"),
      false
    );
  });
