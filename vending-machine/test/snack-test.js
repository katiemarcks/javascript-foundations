...
describe("Snack", function () {
  ...
  it("should be able to remove one item at a time from stock", function () {
    ...
    assert.equal(
      candyBar.removeItem(),
      "Item taken! There are now 6 items left."
    );
  ...
  it("should not be able to remove anything from stock if none left", function () {
    ...
    assert.equal(
      candyBar.removeItem(),
      "Sorry, no hersey's milk chocolate left in stock!"
    );

    assert.equal(
      chips.removeItem(),
      "Sorry, no lay's sour cream and onion chips left in stock!"
    );
  });
});
